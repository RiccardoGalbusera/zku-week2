...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const recipientBalance = await token.balanceOf(aliceWithdrawAddress)
    expect(recipientBalance).to.be.equal(withdrawAmount)
    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
    expect(tornadoPoolBalance).to.be.equal(depositAmount.sub(withdrawAmount))
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const bobWithdrawAddress = '0xDeaD00000000000000000000000000000000BEEf'
    const bobChangeUtxo = new Utxo({ amount: bobSendAmount.sub(bobWithdrawAmount), keypair: bobKeypair })
    await transaction({
      tornadoPool,
      inputs: [bobReceiveUtxo],
      outputs: [bobChangeUtxo],
      recipient: bobWithdrawAddress,
      isL1Withdrawal: false,
    })

    const aliceWithdrawAmount = aliceDepositAmount.sub(bobSendAmount)
    const aliceWithdrawAddress = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48'
    ...
    const bobL2Balance = await token.balanceOf(bobWithdrawAddress)
    ...
    const aliceL1Balance = await token.balanceOf(aliceWithdrawAddress)
